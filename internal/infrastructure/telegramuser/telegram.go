...
func (t *Handler) AiResponse(msg entity.MessageResponse) {
	if msg.ReplyText != "" {
		msg.Message += " reply message: " + msg.ReplyText
	}

	BussinessInfo, err := t.ProductUscse.GetBusinessByPhone(ctx, entity.GetBussinesId{Phone: msg.Phone})
	if err != nil || BussinessInfo.BusinessID == "" {
		log.Println("GetBusinessByPhone error:", err)
		return
	}
	chatID, err := strconv.Atoi(msg.FromID)
	if err != nil {
	...
	SettingsAi, err := t.ProductUscse.GetIntegrationSettingsByOwnerID(ctx, BusinessId, msg.FromID)
	if err != nil {
	...
	if SettingsAi.IsBlocked {
		t.SendTelegramMessage(entity.MessageRequest{
			Phone:  msg.Phone,
			UserID: msg.FromID,
			Text:   "Uzur siz bloklangansiz",
		})
	}
	...
	TokenRequest := EstimateTokenCount(msg.Message + SettingsAi.PromptText + SettingsAi.PromtOrder + string(chatHistoryStr))
	fmt.Println(TokenRequest, SettingsAi.TokenLimit)
	if TokenRequest > SettingsAi.TokenLimit {
		t.sendText(msg, "kechirasiz token koplik qildi")
		return
	}
	firstRes, err := t.GeminiModel.FirstStep(ctx, SettingsAi.IntelligenceLevel, msg.Message, SettingsAi.PromptText, SettingsAi.PromtOrder, history)
	if err != nil {
	...
	if firstRes.IsAiResponse {
		t.sendText(msg, firstRes.AiResponse)
	...
	// 	t.sendText(msg, firstRes.UserMessage)
	// }
	...
	if firstRes.IsProductSearch {
		...
		FoudnProducts, err := t.GeminiModel.ExtractProductName(ctx, msg.Message, SettingsAi.PromtProdcut, listProductStr, history)
		if err != nil {
		...
		ResponseToken += EstimateTokenCount(string(ByteRes))
		TokenRequest += EstimateTokenCount(msg.Message + string(listProductStr) + SettingsAi.PromtProdcut + string(chatHistoryStr))
		//t.sendText(msg, FoudnProducts.)
		err = t.ProductUscse.CreateTokenUsage(ctx, &entity.ClientTokenUsage{
			BusinessID:     BusinessId,
			SourceType:     "telegram",
			UsedFor:        "text",
		...
		if FoudnProducts.IsAiResponse {
			t.sendText(msg, FoudnProducts.AiResponse)
			return
		}
	...
	if firstRes.Action == "get_order_status_all" {
		...
		if OrderRes == nil && err == nil {
			t.sendText(msg, "sizda buyurtmalar mavjud emas")
			return
		}
		if err != nil {
			fmt.Println("Error while GetOrderByID:", err)
	...
	if firstRes.Action == "notification_to_admin" {
		firstRes.Message += fmt.Sprintf("/nuserid: %s, chatid: %d, messageid: %d", UserId, chatID, msg.MessageID)
	...
	if firstRes.Action == "cancel_order" {
		...
		t.sendText(msg, message.Message)
		if message.Isupdate {
			t.SendMessageToAdmin(entity.SendMessageResponse{
				Type: "notification",
				Notifications: &entity.Notification{
	...
	if firstRes.Action == "set_order_location" {
		...
		t.sendText(msg, updateMessage.Message)
		return
	}

}

func (t *Handler) sendErrorMessage(msg entity.MessageResponse) {
	Resp, err := t.SendTelegramMessage(entity.MessageRequest{
		Phone:  msg.Phone,
		UserID: msg.FromID,
		Text:   errorMEssage,
	})
	if err != nil {
		fmt.Println("Error while SendTelegramMessage:", err)
		return
	}
	if Resp.Code != 0 {
		fmt.Println("Error while UpdateClientStatus:", Resp.Message)
		return
	}
	err = t.ProductUscse.CreateChatHistory(ctx, &entity.ChatHistory{
...
func (t *Handler) ClientOrders(orders []*entity.OrderResponseByOrderId, msg entity.MessageResponse) {
...
func (t *Handler) CreateOrder(ctx context.Context, res *entity.CreateOrder, msg entity.MessageResponse) error {
	var products entity.CreateProductOrder
	order := entity.Order{
		UserID:       res.UserId,
	...
	for i := 0; i < len(res.ID); i++ {
		...
		if err != nil || product == nil {
			t.sendText(msg, fmt.Sprintf("❌ Mahsulot topilmadi (ID: %d).", res.ID[i]))
			return err
		}
		products.Id = product.ID
	...
	if orderResponse.Message == "norows" {
		...
		t.sendText(msg, "Ismingizni kiriting:")
		return err
	}
...
func (t *Handler) handleOrderStatus(
	...
	OrderStatus []*entity.OrderStatus, msg entity.MessageResponse,
) (bool, error) {
